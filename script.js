...
// Map tile value → symbol id in sprite
const SYMBOLS = {
...
// Inline sprite into DOM once
async function loadSprite() {
  // If you’re serving with Live Server / Codespaces, this works.
  // If you’re opening index.html from file:// and fetch is blocked,
  // copy the sprite’s raw SVG into index.html inside a hidden <div>.
...
function draw(){
  ...
  const W = tilesEl.clientWidth, pad = 10, gap = 12;
...
// ---- moves ----
function saveHistory(){ state.history = [{grid: cloneGrid(state.grid), score: state.score}]; undoBtn.disabled = false; }
function slideRowLeft(row){
  const arr = row.filter(v=>v);
  for(let i=0;i<arr.length-1;i++){
    if(arr[i]===arr[i+1]){ arr[i]*=2; state.score+=arr[i]; arr[i+1]=0; }
  }
  const res = arr.filter(v=>v); while(res.length<SIZE) res.push(0); return res;
}
function rotate(g){ const m = emptyGrid(); for(let r=0;r<SIZE;r++) for(let c=0;c<SIZE;c++) m[c][SIZE-1-r]=g[r][c]; return m; }
function move(dir){
  saveHistory();
  let g = cloneGrid(state.grid);
  for(let i=0;i<dir;i++) g=rotate(g);
  let moved=false;
  for(let r=0;r<SIZE;r++){ const sl=slideRowLeft(g[r]); if(sl.some((v,i)=>v!==g[r][i])) moved=true; g[r]=sl; }
  for(let i=0;i<(4-dir)%4;i++) g=rotate(g);
  if(!moved){ state.history.pop(); return; }
  state.grid=g; spawnTile(); draw(); checkUnlocksAndRender();
  if(!movesAvailable(state.grid)) showModal('Game Over', `<p>Final score: <strong>${state.score}</strong></p>`);
}
function movesAvailable(g){
  for(let r=0;r<SIZE;r++) for(let c=0;c<SIZE;c++){
    if(g[r][c]===0) return true;
    const v=g[r][c]; if(r+1<SIZE && g[r+1][c]===v) return true; if(c+1<SIZE && g[r][c+1]===v) return true;
  } return false;
}
function reset(){
  state.grid = emptyGrid(); state.score=0; state.history=[];
  spawnTile(); spawnTile(); renderFacts(); draw(); checkUnlocksAndRender();
}
...
function checkUnlocksAndRender(){
  ...
  const newly = [];
  FACT_THRESHOLDS.forEach(t=>{ if (high>=t && !state.unlocked.includes(t)) { state.unlocked.push(t); newly.push(t); } });
  if (newly.length){
    localStorage.setItem(unlockedKey, JSON.stringify(state.unlocked));
    renderFacts(); newly.forEach(t=>state.factQueue.push(t)); runFactQueue();
  } else { renderFacts(); }
}
...
function renderLegendIcons(){
  document.querySelectorAll('.legend li').forEach(li=>{
    ...
    if (!sw) { const cb=li.querySelector('input[type="checkbox"]'); sw=document.createElement('span'); sw.className='swatch'; if(cb) cb.replaceWith(sw); else li.prepend(sw); }
    const id = SYMBOLS[v];
...
window.addEventListener('keydown', (e)=>{
  const k=e.key; if(['ArrowLeft','ArrowUp','ArrowRight','ArrowDown','a','w','d','s'].includes(k)) e.preventDefault();
...
let start=null;
tilesEl.addEventListener('touchstart',(e)=>{ if(e.touches.length===1) start={x:e.touches[0].clientX,y:e.touches[0].clientY}; },{passive:true});
tilesEl.addEventListener('touchend',(e)=>{ if(!start) return; const dx=e.changedTouches[0].clientX-start.x, dy=e.changedTouches[0].clientY-start.y; if(Math.max(Math.abs(dx),Math.abs(dy))>24){ if(Math.abs(dx)>Math.abs(dy)) move(dx<0?0:2); else move(dy<0?1:3);} start=null; });

howBtn.onclick = ()=> showModal('How to Play', `<ol><li>Use arrow keys or swipe to move tiles.</li><li>Equal tiles merge into the next icon tier.</li><li>Build toward <strong>2048</strong> to complete the well.</li></ol>`);
ctaPlay.onclick = newBtn.onclick = reset;
restartBtn.onclick = reset;
undoBtn.onclick = ()=>{ const last = state.history.pop(); if(!last) return; state.grid=last.grid; state.score=last.score; draw(); checkUnlocksAndRender(); undoBtn.disabled=true; };
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    renderLegendIcons();      // build legend
    reset();                  // start game
  } catch (e) {
    console.error(e);
    // Fallback: still start the game even if sprite fetch fails
    reset();
  }
});
/* =========================
   Water 2048 — script.js
   Sprite-only icon rendering
   ========================= */
